...
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The production `.env`     | `SECRETS_KEY` in it is **irreplaceable**: it unlocks every secret in the vault (My secrets, Company and each client's), the mailbox passwords and every two-factor sign-in secret stored in the database — and every client file, each sealed with a key it holds, so without it no file opens, in the bucket or from a backup. `SESSION_SECRET` and `POSTGRES_PASSWORD` can be new (everyone signs in again); `SMTP_PASS` comes from the mail provider. |
...
`.env` is never in a backup, on purpose: together with the database dump it would make every
secret in the vault readable.

**The vault after a restore.** An unlock lives in the app's memory, so a restore or a restart locks
the vault for everyone and each person enters their own password again; nothing needs re-entering.
A secret deleted more than 30 days before the copy was taken is gone; one still in the Trash in the
copy but past its 30 days is removed again by the next night's `secrets:purge`, at 04:40.
