...
/**
 * Convert transaction: create the client + mark the lead won, atomically + race-safe. `alsoInTx`
 * is what must happen with it in the same transaction: the lead's task files are filed into the
 * new client's Internal (files.md §5.6).
 */
export function convertLead<T>(
  leadId: string,
  clientData: Prisma.ClientCreateInput,
  alsoInTx: (tx: Prisma.TransactionClient, clientId: string) => Promise<T>,
) {
  return prisma.$transaction(async (tx) => {
    ...
    const extra = await alsoInTx(tx, client.id);
    return { client, lead, extra };
  });
}
